...
def render_notebook(name: str):
    try:
        ...
        book_url = f"https://https://eds-book.netlify.app/gallery/{name}/notebook"
...
def run_directive(name, data):
    assert name == "edsbook-notebooks-mini"
    return [{"type": "edsbook-notebooks-mini", "children": []}]


def run_transform(name, data):
    with concurrent.futures.ThreadPoolExecutor() as pool:
        # Find our notebook nodes in the AST
        notebook_nodes = find_all_by_type(data, "edsbook-notebooks-mini")
